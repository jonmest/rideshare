...
update-driver-location driver_id="driver-1":
  curl -s -X POST http://127.0.0.1:3000/driver/{{driver_id}}/location \
    -H 'Content-Type: application/json' \
    -d '{"driver_id": "{{driver_id}}", "lat": 40.7128, "lon": -74.0060, "update_time": 1710000000}' | jq .

update-driver-availability driver_id="driver-1":
  curl -s -X POST http://127.0.0.1:3000/driver/{{driver_id}}/availability \
    -H 'Content-Type: application/json' \
    -d '{"driver_id": "{{driver_id}}", "status": "available", "update_time": 1710000000}' | jq .
